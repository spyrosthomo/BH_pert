''' 
    Modified potential as in [t2], [t3]
        V = Vgr + \sum_{k=0}a(k) f(r)/rs^2 * (rs/r)^2
        ...
        - sigma       : compactification of 1.) sigma = 0: electromagnetic perturbations
                            ...
                            sigma=1-spin^2
        - rs: Horizon
        - a : coefficient for the potential 
'''
...
class Potential:
    import inc 
    #----------------------------
    def __init__(self, l, s, rs=1, a2=1):
        import numpy as np 
        #-------
        self.potentialName = "GR k2"
        self.potentialForm = f"V = f(r)({l}({l}+1)/r^2 + {s}rs/r^3) + f(r)a1*(rs/r)^2/rs^2"
        self.forFileNames  = f"GRaxial{int(s)}_k2"
        self.saveLoc       = "../figures/GR_k2.pdf"
